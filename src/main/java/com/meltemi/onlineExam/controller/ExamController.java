...
import com.meltemi.onlineExam.service.ClasseService;
import com.meltemi.onlineExam.service.ExamService;
import com.meltemi.onlineExam.service.PaperService;
...
public class ExamController {
    ...
    public String toExam(@PathVariable ("id") Integer id,Model model){
        List<Exam> Exams = examService.getAllById(id);
    ...
    public String submitExam(Integer paperId, Integer studentId, HttpServletRequest request){
        System.out.println("学生的id为:"+studentId);
        //判断学生是否已经参加过考试
        if (recordService.queryStudentAndPaper(studentId, paperId).size() >= 1) {
            return "/commons/404";
        }
        else {
            try{
                ...
                record.setRecordData(request.getParameter("record_data"));
                recordService.addRecord(record);
                return "redirect:/exam/toHist/"+studentId;
